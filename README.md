...
`vendas.html` é a nova página de vendas em revisão. Ela mantém a landing de
captação em `index.html` intacta e reaproveita as demonstrações reais da pasta
`media/`.

Antes de publicar a página de vendas:

1. inserir os links Hotmart dos planos mensal e anual em `CHECKOUTS`, no
   `vendas.js`;
2. revisar preços e condição de fundador;
3. promover `vendas.html` para a rota pública somente após aprovação.
